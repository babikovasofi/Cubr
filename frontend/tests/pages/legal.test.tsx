...
// сервер, мест/рейтинга нет, почта не публикуется никогда) и перелинковку
// страниц.
...
describe("PrivacyPage", () => {
  ...
  it("перечисляет хранимое и обещает не публиковать почту (ник — опционально видимый другим)", () => {
    renderPage(<PrivacyPage />);

    expect(screen.getByRole("heading", { name: "Что хранится на сервере" })).toBeTruthy();
    expect(screen.getByText(/Почта не показывается/)).toBeTruthy();
  });
