    ...
    <div className="flex flex-col gap-12">
      ...
      <section className="flex flex-col items-start gap-6 sm:flex-row sm:items-center sm:justify-between sm:gap-8 lg:gap-10">
        <div className="flex min-w-0 flex-col gap-5">
          ...
          {/* Требование «нужен компьютер» — не сноска, а условие входа: человек,
              прочитавший его после регистрации с телефона, уже потерян. Поэтому
              не `small faint` внизу блока, а плашка в размер основного текста:
              заливка `surface-2`, граница `line`, жирный `ink` (§1 — цвет тут
              не нужен, крупности и контраста достаточно). */}
          <p className="max-w-prose rounded-md border border-line bg-surface-2 px-4 py-3 font-sans text-body font-bold text-ink">
            {handheld
              ? t(
      ...
      {/* Ссылка на правила — внизу страницы, тихо: она справочная, не должна
          перетягивать взгляд на первом экране (owner). */}
      <Link to="/rules" className="font-sans text-small font-bold text-muted">
