// De-ranked daily participation board (plan: daily-scramble, mirrors
// TournamentStandings). Presentational only — all fetching lives in
// useDailyBoard. Renders rows in the order the API returns them
// (submitted_at ASC); NEVER computes or shows a rank/position number — that
// is the point of "participation, not ranking".
//
// Privacy (П10): every row is built from `display_name` alone (already
// "Аноним" substituted server-side for an unset public_handle). This
// component never receives, and must never render, email or the account
// nickname.

import Button from "../components/Button";
import Spinner from "../components/Spinner";
import type { DailyBoardEntry, DailyBoardRead } from "../api/daily";
...
function Row({ entry }: { entry: DailyBoardEntry }) {
  const t = useT();
  return (
    <div
      className={[
...
export default function DailyBoard({ data, loading, error, reload }: DailyBoardProps) {
  const t = useT();
  return (
    <section className="flex flex-col gap-4 rounded-lg border-2 border-ink bg-surface p-7">
      <div className="flex flex-col gap-1">
        <span className="font-sans text-overline uppercase text-muted">
          Скрамбл дня{data ? ` · ${data.day_label}` : ""}
        </span>
        <h3 className="font-sans text-h3 text-ink">{t("Кто уже собрал сегодня")}</h3>
      </div>
          ...
          {data.entries.length === 0 ? (
            ...
            <div className="flex flex-col gap-2">
              {/* No stable id on the wire (locked contract, П10/de-ranked — see
                  api/daily.ts DailyBoardEntry): the whole list is replaced
