...
export type DropReason =
  ...
  | "orientation"
  // Показана не та грань, чем требует порядок строгой хватки: центр съёмки сел
  // ТОЧНО на чужой эталон.
  //
  // Это ДРОП, а не пересъёмка, и разница здесь принципиальная. Проверка знает
  // о показанной грани ровно одно — цвет её центра, — и потому не может
  // отличить «человек взял не ту грань» от «зрение промахнулось по центру
  // верной». Второе — это red↔orange, главный hotspot R1, то есть ровно та
  // величина, которую гейт и меряет. Пересъёмка выбрасывала бы такие съёмки
  // мимо числителя и знаменателя, и механизм умел бы только УЛУЧШАТЬ цифру:
  // 53/54 превращались бы в 54/54 без следа. Дроп кладёт их в знаменатель и в
  // drop-rate, где потолок 15% — то есть систематический промах центра
  // проваливает условие честно, вместо того чтобы вычистить себя из замера.
  | "wrong-face";
