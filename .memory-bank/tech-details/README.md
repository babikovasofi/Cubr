...
## Разбиение бандла (реализовано 2026-08-05)
Входной чанк **644 kB → 273 kB** (gzip 193 → 84). Ленивые (`React.lazy` в `App.tsx`) — ровно
ритуальные роуты (solo, турнир, день, дуэли, онбординг, профиль) и правила/приватность; лендинг,
вход-регистрация и текст шапки/футера остаются в первом экране, чтобы он не мигал спиннером.
- **Что вынеслось:** MediaPipe (руки) + cubejs (солвер) — общий чанк ~189 kB, грузится при первом
  заходе на ритуальный экран. Анониму на лендинге он больше не приезжает.
- **Инвариант держит `frontend/scripts/check-bundle.mjs`** (npm `postbuild`): бюджет входного чанка
  320 kB И отсутствие в нём маркера `HandLandmarker`. Второе важнее: один статический
  `import` камеры из неленивого модуля молча возвращает всё обратно, а по логу сборки это некому
  заметить. Бюджет поднимать осознанно, в самом скрипте.
- **`components/RouteErrorBoundary.tsx`** — новый режим отказа, которого не было у одного бандла:
  чанк не доехал (сеть моргнула / выкатили новую версию при открытой вкладке). React снимает
  поддерево, человек видит белый экран. Граница объясняет и предлагает перезагрузку.
- Не тронуто: `i18n/en.ts` (~50 kB) грузится всегда, хотя нужен только при `lang=en` — ленивая
  загрузка словаря упирается в синхронный `translate()`, который зовут чистые модули и тесты.
