...
- **Этап 6 — лендинг + правила/приватность** ✅ код + тесты (frontend-only, статика) — `frontend/`.
  ...
  - ✅ frontend vitest **503** (+20: HomePage anon/authed/loading-развилка + честностные утверждения
    текстов), tsc/eslint/build чисто. Инлайн-цикл без субагентов.
...
- **Этап 6 — остаток прод-подготовки:** экран-заглушка для мобильных браузеров, фильтр слов для
  ников, деплой (Vercel + Railway/Render, домен, HTTPS, SPF/DKIM, бэкапы), внешний cron на
  `python -m app.jobs.finalize`, аналитика воронки, закрытый тест на 3–5 куберах.
