...
## Тренажёр PLL (V3, 2026-08-18) — PR #21, не смёржен

Ветка `feat/ll-trainer` от свежего `main` (не от `fix/vision-stickerless-facefit` —
та несёт незакоммиченную отладку зрения). Роут `/trainer`: выбор одного или нескольких
PLL-кейсов → скрамбл, который гарантированно оставляет ровно этот кейс.

- **Покрытие: закрытый набор всех 21 PLL. 0 OLL** — сознательно не начато, не «частично».
- **Без солвера**: `скрамбл = [ориентация] + invert(алгоритм_кейса) + AUF`. cubejs —
  только в тестах; рантайм-чанк `/trainer` — данные (строки алгоритмов + фаселетов),
  12.36 kB gzip 3.89 kB, лениво подгружается, cubejs в него не утекает.
- **Каждый алгоритм доказан тестом, не на глаз**: независимая спецификация циклов
  углов/рёбер каждого кейса выведена перебором всех 288 легальных состояний последнего
  слоя (288 → ровно 22 класса: 1 собранный + 21 PLL) — ДО того, как вписан хоть один
  алгоритм. Настоящие алгоритмы (конвенция speedsolving.com wiki) сверены с этой
  спецификацией перестановочным равенством, а не глазами.
- Публичный роут, без `DesktopOnlyGate` (нет камеры/таймера/сокета — единственный
  режим, которому телефон только помогает) и без `ProtectedRoute` (ноль пользовательских
  данных). Ноль сетевых вызовов со страницы, ноль строк в `solves` (§П5).
- Тесты: frontend vitest **787 → 1042** (+255), все зелёные; tsc/eslint/prettier чисто;
  build проходит, бюджет входного чанка не задет.
- Самое неуверенное решение: буквенные названия кейсов (Ga–Gd, Ja/Jb, Ua/Ub) в паре
  мест расходятся между источниками; корректность самого алгоритма доказана тестом
  независимо от подписи, в UI диаграмма (выводится из состояния) авторитетнее буквы.
- Отчёты: `swarm-report/ll-trainer-plan.md`, `swarm-report/ll-trainer-build.md`.
  PR: https://github.com/babikovasofi/Cubr/pull/21 — не смёржен, не задеплоен.

## Тренажёр OLL (2026-08-18) — расширение тренажёра PLL, PR #25, не смёржен

Ветка `feat/oll-trainer` от актуального `main`. `/trainer` теперь покрывает оба набора
последнего слоя: переключатель PLL/OLL/оба, выбор случаев внутри каждого набора,
обратная совместимость с уже сохранённым PLL-only выбором (старые ключи
`cubr.trainer.pll.cases`/`.anyGrip` читаются как миграция, новые пишутся под новыми
ключами `cubr.trainer.sets`/`cubr.trainer.cases`/`cubr.trainer.anyGrip`).

- **Покрытие: полный закрытый набор всех 57 OLL** (плюс уже имевшиеся 21 PLL).
  Перечисление 216 легальных состояний ориентации последнего слоя (3 положения на
  каждый из 4 углов × 2 на каждое из 4 рёбер, с законами сохранения) дало **ровно 58
  классов: 1 собранный + 57 OLL** — проверено дважды независимо (черновым скриптом на
  cubejs до единого алгоритма в файле, и тем же перебором внутри
  `ollCompleteness.test.ts`), число совпало с классическим с первого раза после одной
  реальной ошибки в модели эквивалентности (см. ниже).
- **Реальная находка, не подогнанная**: группа эквивалентности для ориентации — **4
  элемента** (один поворот U), а не 16, как у перестановок PLL — у чистой ориентации
  нет осмысленного «домножения слева» (после identity-перестановки такое домножение
  выводит состояние за пределы пространства 216, а не даёт другое представление того
  же кейса). Подтверждено на реальных `co`/`eo` от cubejs, а не выведено на бумаге.
  Задокументировано в `oll.ts` и `model.ts`.
- **Каждый алгоритм доказан тестом**: как и для PLL — сначала независимая спецификация
  (`cornerTwist`/`edgeFlip`) из чистого перебора, потом 57 алгоритмов (нумерация и
  конвенция speedsolving.com wiki) сверены с ней структурным равенством, без единого
  расхождения (0 непойманных, 0 дублей на первом верном перебора).
- **Вторая реальная находка**: в отличие от PLL, настоящие OLL-алгоритмы **не**
  сохраняют перестановку последнего слоя (только ориентацию) — все 57 при применении
  из собранного состояния переставляют угол/рёберные кубики. Поэтому диаграмма OLL
  (`OllDiagram`) красит наклейки бинарно (сориентирована/нет), а не реальными цветами
  граней, как диаграмма PLL, — реальные цвета там были бы случайным шумом конкретного
  алгоритма, а не диагностическим сигналом.
- **Общий код с PLL не форкнут, а обобщён**: `generate.ts` (`pickCase`/
  `generateCaseScramble`) типизирован дженериком `ScrambleCase`; общая сетка диаграммы
  вынесена в `LastLayerGrid.tsx`, `LastLayerDiagram` стал десятистрочной обёрткой над
  ней. Оба изменения — чистое расширение типов/разбиение вёрстки; тесты PLL (
  `generate.test.ts`, `LastLayerDiagram.test.tsx`) прогнаны без изменений и зелёные.
  `pll.ts`, `pllCompleteness.test.ts`, `pllTable.test.ts` не тронуты вовсе.
- Тесты: frontend vitest **1042 → 1665** (у автора отчёта после коммитов), все зелёные;
  tsc/eslint/prettier чисто; build проходит, `TrainerPage` остаётся своим ленивым
  чанком (29.87 kB / 7.2 kB gzip), cubejs в рантайм не просочился.
- Самое неуверенное решение: никнеймы кейсов (Sune, Bowtie, Squeegee…) взяты одним
  запросом с speedsolving.com wiki и не перепроверены независимо — в отличие от самих
  алгоритмов (доказаны структурно) это чисто косметическая подпись; `id`/`number`
  (1–57) и диаграмма остаются источником истины.
...
## Гейт 0.3 — живая сессия 2026-08-20 (ветка `vision/stickerless-session`, не слита)

**Зрение работает.** Это первая сессия, где вопрос «читает ли камера stickerless»
закрыт положительно, и закрыт числами, а не впечатлением.

**Санити (собранный кубик, строгая хватка, led, stickerless): семь чтений подряд по
54/54.** 378 наклеек, ноль ошибок, `red↔orange 0 из 126`, `white↔yellow 0 из 126`,
дропов ноль, пересъёмок ноль. **Скрамбл-режим: два чтения 54/54**, то есть с
настоящими соседствами цветов.

**Оставшиеся промахи — не зрение.** Два чтения по 49/54 разобраны поимённо: все
промахи сидели на одной съёмке (грань U) при ΔE ячеек 1.1–4.3, то есть камера
прочитала ровно то, что было в кадре, а в кадре была не та грань, которую ждал
эталон (скрамбл собран не тот либо грань показана повёрнутой). Единственная
настоящая ошибка зрения за серию — одна R→L, и её причина названа: красный
эталон снялся розовым (`RGB(227,116,129)`, ΔE до анкора 16.5), настоящий красный
оказался ближе к оранжевому. Дропы 33% — обе человеческие (`wrong-face`,
`orientation`), инструмент поймал их верно.

**Что чинилось по ходу (в порядке вклада).**
- `b992527` **разделение метрик**. Три промаха «синий прочитан белым» оказались
  разницей СВЕТА, а не цвета: `RGB(89,159,249)` при эталоне `RGB(9,101,207)`, тот
  же тон, светлота выше на два десятка. У CIEDE2000 для этого есть штатный
  параметр `kL`; при `kL=2` те же ячейки дают 11.0 до синего против 13.7 до
  белого. Метрика РАЗДЕЛЕНА, а не заменена: «какой из шести» (argmin, квоты,
  раскладка центров, отрыв) — ослабленная светлота (`colors.deltaEClassify`,
  `config.CLASSIFY_KL`); «наклейка ли это вообще» (`FACE_MAX_MEDIAN_DE`,
  `STICKER_MAX_DELTA_E`, разделимость эталонов, стоимость подгонки) — обычная,
  потому что именно светлота отличает щель и стол от наклейки. Цена измерена:
  ближайшая пара эталонов 16.3 → 14.4 при пороге вердикта 10.
- `a41b3b3` **детектор пальцев по хроме YCbCr** (границы Chai–Ngan: 77 ≤ Cb ≤ 127,
  133 ≤ Cr ≤ 173). Кожа в Lab лежит рядом с оранжевым, argmin уверенно отвечал
  «оранжевый» — отсюда все прошлые ошибки `R→L` в одну сторону. Съёмка с ячейкой,
  закрытой больше чем наполовину, теперь отказывается. Плюс телеметрия камеры в
  отчёте: **ручные экспозиция и баланс белого на этой машине не поддерживаются**
  (Chrome/macOS), значит автобаланс гуляет между калибровкой и чтением — это
  подтверждено, а не гипотеза.
- `dfeefea` **цветовая коррекция матрицей 3×3 по шести центрам** (МНК в линейном
  RGB, регуляризация к единичной: на вырожденных образцах коэффициенты без неё
  доходят до 63, с ней ≤1.9). Только продуктовый путь; гейт по-прежнему меряет
  сырое чтение.
- `7671394` **зажатый штраф щели** (у stickerless окантовка бывает ЯРЧЕ наклейки,
  и незажатый штраф уводил сетку с кубика на стол при вердикте «ПОДОГНАНА») и
  **`edgeContrast` вызывался с чужим аргументом** — мерил границы полосой вчетверо
  шире той, которой штрафует `regionCost`.
- `5533fb0` **«не та грань» — дроп, а не пересъёмка**: проверка знает только цвет
  центра и потому не отличает ошибку человека от промаха зрения по центру; как
  пересъёмка она вырезала из гейта ровно те ошибки, которые гейт меряет.
- `9d6cd69` **санити не закрывает гейт** и говорит это сам. Гейт считается только
  по скрамбл-условиям; санити-строка получает вердикт `[САНИТИ]` вместо PASS/FAIL
  и причину вместо «нужно ещё N чтений».
- `a9caf8c` **отчёт называет пару, которую спутает классификатор**, а не ту, что
  ближе по обычной метрике: на пересвеченном синем это разные пары (U–B 14.5
  против R–L 15.5). Вердикт калибровки тоже считается по решающей метрике.
- `e904409` **почерк «спорит не зрение»**: промахи в одной съёмке при чистых цветах
  теперь называются словами, чтобы человек не шёл чинить свет.

**Практические пороги, добытые замерами.**
- Перед серией смотреть **второе** число в строке разделимости («в метрике
  выбора»): **≥16 — работаем**. На санити и 13.3 давало 54/54, но там красный с
  оранжевым не встречаются в одном кадре; на скрамбле решает именно оно.
- `R: ΔE до анкора` **> 10 — красный снялся бледным**, перекалиброваться. В
  здоровых прогонах было 1.5–2.9, в сорвавшемся — 16.5.
- Хват за боковые рёбра; «задетых краем пальца» до 9 ячеек за чтение точности не
  портило, закрытых наполовину не было ни разу.

**Что осталось до гейта.** 20 чтений в условии `scramble | fixed` при drop ≤15%.
Набрано 4 (две по 54/54, две по 49/54 из-за расхождения кубика с эталоном),
drop 33% — обе причины человеческие. То есть остаётся набрать объём аккуратно,
а не чинить код.

## Гейт 0.3 — живая сессия 2026-08-19 (ветка `diag/accuracy-cell-dump`, от `main`, не слита)

Первая сессия, где зрение **читается**, и где стало видно, обо что оно спотыкается на самом
деле. Шесть коммитов, frontend vitest **1750**, tsc/eslint/prettier чисто. PR не открывался.

**Главный результат по числам.** Санити-чтение (собранный кубик, строгая хватка, `led`,
stickerless): **45 из 45** на пяти гранях; `red↔orange 0 из 18`, `white↔yellow 0 из 18`.
Шестая грань дала девять ошибок, потому что человек показал жёлтую вместо оранжевой — это
ошибка условия замера, не зрения (см. «не та грань» ниже). На скрамбле лучшее сырое чтение —
**50/54 = 92.6% PASS**, накопленное по трём чтениям 87.7% (Wilson-LB 81.7%), n=3 из нужных 20.

**Гипотеза «оранжевый центр читается белым» закрыта — она была неверна.** Полный отчёт с
эталонами (просился пять раз, получен здесь впервые) показал здоровую калибровку:
`B RGB(0,88,200)`, до белого 41 единица; `min попарный ΔE 16.2 (R–L)` при пороге вердикта 10.
Цвета камера различает. При этом хорошая калибровка — не константа сессии: у трёх подряд
калибровок `min ΔE (R–L)` вышло 16.2 / 16.1 / 14.7, а в сессии до того — 13.5, и на 13.5
красный сыпался. **Смотреть это число перед каждой серией.**

**Настоящие источники ошибок (по живым RGB, а не по догадкам):**
1. **Пальцы в кадре — половина промахов.** Ячейки `RGB(233,169,141)`, `(186,129,107)`,
   `(226,152,131)`, `(190,147,107)` — телесный тон. Ближайший к нему цвет кубика — оранжевый,
   отсюда `red↔orange 3 из 54`, все три R→L и ни одного L→R: это не путаница красного с
   оранжевым, это палец, засчитанный оранжевым. Счётчик «ближе к телесному тону» давал 3–4 на
   чтение. Лечится хватом за боковые рёбра, без единой строчки кода: те же три чтения дали бы
   ~94% / 98% / 92.6% вместо 85.2 / 92.6 / 85.2.
2. **Сетка заходит на соседнюю грань.** На СОБРАННОМ кубике грань D дала ячейки
   `RGB(255,129,50)` при оранжевом эталоне `(252,123,64)` — край сетки заехал на соседнюю
   грань. На зелёной грани — `(198,192,56)`, `(153,187,52)`: жёлтый и зелёно-жёлтые смеси,
   то есть граница. Причина видна в телеметрии: **у stickerless щели не темнее наклеек, а
   светлее** — `контраст щелей −2.7 … −7.7` при цели 12. Подгонке не за что цепляться.
3. **Блик на ЧАСТИ ячейки флага не поднимает.** `kept` считает выбитой только ячейку, съеденную
   пересветом целиком (порог 25%); половинный блик тянет тёмно-синюю наклейку к белому молча.

**Что сделано в инструменте.**
- `f85bff7` **дамп всех 9 ячеек каждой съёмки** при браке (сырой argmin, 3 ряда, центр в
  скобках, строчная буква = ΔE > 34) + **брак попадает в «Копировать отчёт»**. До этого дроп жил
  строкой на экране и умирал со следующей съёмкой: прогон, где в дроп ушло всё, копировался как
  сводка нулей. Все шесть веток отказа сведены в один `dropRead()`.
- `88aaf90` **замок по решётке**: съёмка, у которой ОБА признака (щели и границы) обвалились
  ниже `FACE_FIT_COLLAPSE_FRAC = 0.35` от медианы соседних граней ТОГО ЖЕ чтения, не
  принимается. Порог относительный, потому что абсолютного не существует: в одном прогоне
  здоровая грань дала щели 3.3, а сломанная 4.0 — разделяет только контекст. Нужны оба признака
  (грань с одноцветными соседями законно даёт низкие границы), бюджет — одна пересъёмка на
  грань, при обвале у всех шести замок молчит. Ловит случай, недоступный `FACE_MAX_MEDIAN_DE` по
  устройству: белый стол за краем рамки — законный цвет кубика.
- `e07c602` **баланс цветов первой строкой брака**: на кубике ровно по 9 каждого. Отказ
  «физика не определила поворот граней» на живых данных означал `R 5, L 12, U 12` — четыре
  красных прочитаны не красными; физика была следствием, а не причиной.
- `6525855` в строке подгонки печатаются **оба** признака (`щели/границы`): у stickerless щели
  штатно в нуле, и одно число читалось как приговор геометрии там, где решётку держат границы.
- `b7bdc25` **худшая ячейка по доле выживших пикселей** в каждой съёмке + строка подгонки
  прикладывается ко всем шести отказам, а не к одному.
- `36f226f` «Копировать отчёт» больше не гаснет, когда в прогоне нет ни одного засчитанного
  чтения — то есть ровно в том случае, ради которого брак и начали запоминать.
- `e228e00` **«показана не та грань» — отказ, а не замечание** (только строгая хватка, она одна
  задаёт порядок U R F D L B). Живой случай: жёлтая грань вместо оранжевой превратила 45/45 в
  45/54. От дрейфа света отличается НАПРАВЛЕНИЕМ, а не величиной: свет уводит центр от всех
  шести эталонов сразу, чужая грань садится точно на другой — потому условий два (далеко от
  ожидаемого И уверенно близко к чужому). Пересъёмка вместо дропа.
- **Счётчик пересъёмок в сводке** — прямое следствие предыдущего. Пересъёмка не идёт ни в
  числитель, ни в знаменатель гейта, значит ею можно тихо накрутить точность: зрение, стабильно
  путающее центр красной грани, теряло бы неудобные съёмки по одной. Одна-две за прогон — человек
  сбился, двадцать — цифре гейта верить нельзя.

**Открытый хвост.**
- Замок на телесный тон (съёмка с пальцем на грани — отказ) обсуждён, **не сделан**: ждёт
  решения, снимать ли сначала серию с новым хватом.
- Свес сетки на соседнюю грань не чинится ничем: подгонке на stickerless не за что цепляться,
  нужна отдельная задача (сжатие области или другой признак края).
- Серия ≥20 чтений на условие не набрана ни разу (максимум n=3): гейт по-прежнему открыт.

**Рабочее правило, вылезшее болью.** Правка фронта = HMR-перезагрузка = обнулённый прогон
харнесса. Часть съёмок этой сессии потеряна ровно так. Пока человек меряет — фронт не трогаем;
правки копятся пачкой и катятся по его команде.

## Planned (следующий фокус)
- **Этап 6 — деплой на VPS (2026-08-18, ночной прогон)** ✅ живой прод на настоящем домене.
  - **Хостинг сменился: собственный Selectel VPS**, не Railway/Vercel. Причина решения —
    `swarm-report/deploy-vps-plan.md`. Три контейнера (`db`/`api`/`caddy`) в одной docker-сети
    на одной машине, `deploy/docker-compose.yml` + `deploy/Caddyfile` + `deploy/.env.example`,
    раннбук переписан в `docs/deploy.md`. `backend/railway.json` больше не путь деплоя, но
    оставлен — документирует то же ограничение `numReplicas: 1`.
  - **Адрес прода — `https://cubr-game.ru`** (проверено 2026-08-23: A-запись → 135.106.181.244,
    сертификат живой, `/` отдаёт 200, `/api/health` 200, `www.` → 301). Переключение с
    fallback-адреса состоялось; ночью 18-го сайт стоял на `135-106-181-244.sslip.io`, потому
    что A-записи ещё не было (`swarm-report/night-deploy-2026-08-18.md`).
  - **Старый fallback `https://135-106-181-244.sslip.io` больше не открывается** — и это не
    поломка. Caddy выпускает сертификат под `SITE_DOMAIN`, теперь это `cubr-game.ru`, а на
    чужой SNI отвечает `tlsv1 alert internal error`. Сервер при этом жив: TCP на 443
    соединяется, порт 80 отдаёт 308. Ссылку показывать только доменную.
  - **Выкатка фронта — руками, автодеплоя нет.** В `.github/workflows/` только `ci.yml`; мерж
    в `main` прод НЕ трогает. Фронт: `npm run build` + `rsync -az --delete frontend/dist/
    deploy@135.106.181.244:/srv/cubr/www/`. Проверять по имени чанка: `assets/index-*.js` в
    выдаче сайта обязано совпасть с локальным `dist/index.html`, иначе на проде старая сборка
    (живой случай 2026-08-23: файлы на сервере оставались от 20 августа).
  - **Реплика ровно одна, это не тюнинг:** комнаты дуэлей в памяти процесса, поэтому
    `WEB_CONCURRENCY=1` + `--workers 1`; `main.py` отказывается стартовать при >1.
    Горизонтальное масштабирование = сначала вынести состояние комнат в Redis.
  - **`.env.example` больше не расходится с `config.py`** — `backend/tests/test_env_example.py`
    сверяет список в обе стороны.
  - ✅ Миграции 0001…0010 накатились на пустую прод-базу чисто. Смоук 10/10 автоматизируемых
    пунктов прошли (регистрация/вход/cookie-флаги/scramble→solve/daily authed-vs-anon/рейт-лимит).
    RU/EN-переключатель — не проверен живьём (клиентское состояние, вне скоупа curl-смоука).
  - ⏳ Осталось человеку: Resend (SPF/DKIM), Google OAuth (redirect сформирован под домен;
    зарегистрирован ли он в консоли — не проверено), живая проверка камерой/дуэлью, закрытый
    тест на 3–5 куберах. A-запись `cubr-game.ru` — сделана.
- **V3 — личная аналитика сборок «Коуч»** ✅ код + тесты (frontend-only, read-only) — `frontend/`.
  - ✅ `profile/coach.ts` + `CoachCard` в профиле: разброс (межквартильный размах валидных
    времён), тренд (медиана последних 5 против предыдущих 5), доля DNF + её тренд, лучшая/
    типичная/худшая сборка окна + отметка «возможно, повезло» при большом отрыве рекорда от
    типичного. Считается из уже загруженной `listSolves(50)` (приём `goals.ts`/`average.ts`):
    ноль новых запросов/эндпоинтов.
  - ✅ **Честная рамка:** никакого разбора по фазам (крест/F2L/OLL/PLL) — заблокирован R1
    (зрение по видео), не изображается. Сознательно НЕ показано время суток/день недели —
    при окне ≤50 сборок корзины (24 часа / 7 дней) почти всегда меньше порога осмысленности.
  - ✅ **Порог осмысленности `MIN_SAMPLE=5`** (та же величина, что `AVERAGE_SIZE`/
    `STREAK_TARGET`): все числа `null`, пока данных меньше; тренд требует двух полных окон
    (10) — «12% на пяти сборках» из брифа закрыто структурно, не эвристикой процента. DNF —
    не-попытка для арифметики времени (не «медленная сборка», как в `average.ts`), но
    попытка для доли DNF; `rejected` не-попытка везде (как в `daily/streak`). Медиана, не
    среднее — выброс не переворачивает тренд.
  - ✅ frontend vitest **1686** (+22: 16 логики, 6 рендера), tsc/eslint/build/prettier чисты.
    Входной чанк 314.7/320 КБ (было 312, +2.7 КБ словаря). [swarm-report/coach-analytics-*].
    PR [#28](https://github.com/babikovasofi/Cubr/pull/28) — ⏳ ждёт зелёного CI и ревью,
    не слит.
