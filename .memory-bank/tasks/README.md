...
- **Локализация RU/EN — проход 1** ✅ код + тесты (frontend-only) — `frontend/`.
  ...
  - ✅ **Проход 2:** профиль целиком (рекорды, настройки, витрина, цели, Ao5, история, график,
    ...
    Осталось русским (проход 3): `vision/guide.ts`, `scramble/moveCopy.ts`, `accuracyRun.ts`,
    длинные `/rules` и `/privacy` — там связная проза и нотация, переводить надо целиком.
...
- **Этап 6 — остаток прод-подготовки:** деплой (Vercel + Railway/Render, домен, HTTPS, SPF/DKIM,
  бэкапы), внешний cron на `python -m app.jobs.finalize`, закрытый тест на 3–5 куберах.
