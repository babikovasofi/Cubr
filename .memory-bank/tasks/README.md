...
**Хвост инструмента.** Подпись «min попарный ΔE … если мало ~<20 — эталоны слиплись»
расходится с порогом вердикта `CALIB_MIN_SEPARATION_DE=10`: на 12.7 текст пугает, вердикт
пропускает. Два числа в одном блоке — поправить.
