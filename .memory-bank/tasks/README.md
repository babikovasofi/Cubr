...
- **Этап 6 — деплой на VPS (2026-08-18, ночной прогон)** ✅ живой прод, ⏳ настоящий домен.
  ...
  - **Сайт живой прямо сейчас, но на fallback-адресе:** `https://135-106-181-244.sslip.io` —
    A-запись `cubr-game.ru → 135.106.181.244` ещё не появилась к моменту прогона. Все 4
    origin-переменных (`SITE_DOMAIN`, `CORS_ORIGINS`, `DUEL_ALLOWED_WS_ORIGINS`,
    `FRONTEND_URL`) + `GOOGLE_OAUTH_REDIRECT_URL` указывают на sslip.io согласованно.
    Переключение на настоящий домен — задача на утро, см. `swarm-report/night-deploy-2026-08-18.md`.
  ...
  - ⏳ Осталось человеку: A-запись `cubr-game.ru`, Resend (SPF/DKIM), Google OAuth
    (redirect уже сформирован под нужный домен, но не зарегистрирован в консоли), живая
    проверка камерой/дуэлью, закрытый тест на 3–5 куберах.
