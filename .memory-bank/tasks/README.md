...
- Отчёты: `swarm-report/friends-plan.md`, `swarm-report/friends-build.md`.
  PR: https://github.com/babikovasofi/Cubr/pull/29 — ✅ слит в `main`, ✅ на проде (2026-08-19,
  `/friends/*` видны в прод-openapi).

## Planned (следующий фокус)
- **Этап 6 — деплой на VPS (2026-08-18, ночной прогон; домен и первая выкатка скриптом — 2026-08-19)** ✅ живой прод на своём домене.
  ...
  - **Сайт живёт на `https://cubr-game.ru`** (2026-08-19). A-запись появилась, сертификат
    выдан, origin-переменные переключены: preflight с `Origin: https://cubr-game.ru` отдаёт
    `access-control-allow-origin`, тот же запрос с sslip.io-origin — уже нет. Fallback-адрес
    `https://135-106-181-244.sslip.io` больше не отвечает и является историей (как он
    поднимался — `swarm-report/night-deploy-2026-08-18.md`).
  - **Первая выкатка `deploy/scripts/deploy.sh` — 2026-08-19, успешна** (прод = `main`
    на коммите `531b317`). Entry-чанк сменился `index-DUKYm8eP.js` → `index-DXHavcJ_.js`,
    смоук главная/`/api/health`/`/rules` 200. Новых миграций не было — база уже на 0011.
  ...
  - ⏳ Осталось человеку: Resend (SPF/DKIM), Google OAuth
...
- **V3 — личная аналитика сборок «Коуч»** ✅ код + тесты (frontend-only, read-only) — `frontend/`.
  ...
  - ✅ frontend vitest **1686** (+22: 16 логики, 6 рендера), tsc/eslint/build/prettier чисты.
    ...
    PR [#28](https://github.com/babikovasofi/Cubr/pull/28) — ✅ слит в `main`, ✅ на проде
    (2026-08-19).
