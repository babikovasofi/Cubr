...
**Текущая работа (2026-08-05):** ветка `fix/vision-stickerless-facefit` — первый живой заход на
гейт 0.3. Третий режим счёта «по картинке» (позиции с точностью до поворота грани, поворот из
физики кубика), геометрия для stickerless, четыре бага диагностики. Стикерный даёт 100% на
дневном и на LED; stickerless под LED не дал ни одного чтения — центр оранжевой грани читается
как белый. Путь гейта намеренно не тронут. Ждём полный отчёт с эталонами калибровки.
