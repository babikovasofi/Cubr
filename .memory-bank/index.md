...
закрыт) — код готов (review ship).
Этап 3 честность спивотили: остаток (frames+OpenCV, event-stream) блокирован R1/камерой — вернёмся
после ресурса. Открыты гейт 0.3 (точность ≥90%), manual QA камеры/соло, остаток Этапа 6 (фильтр
ников, деплой). Детали: tasks/README.md.
