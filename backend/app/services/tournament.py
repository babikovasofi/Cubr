...
def display_name_for(handle: str | None) -> str:
    """The de-ranked board's display name for a user.

    ALWAYS ``User.handle`` (deliberately, opt-in set by the user) or the
    literal "Аноним" when unset — NEVER the account email (П10 / privacy).
    Callers must not substitute any other user field here.
    """
    return handle or ANONYMOUS_DISPLAY_NAME
...
async def get_current_standings(
    ...
    NEVER selects ``User.email`` — only ``User.handle`` (П10). No
    tournament yet this week -> empty board, zero counts, ``your_entry``
    ``None`` (never a 404/500).
    ...
    entries_result = await session.execute(
        select(TournamentAttempt.user_id, TournamentAttempt.time_ms, User.handle)
    ...
    your_result = await session.execute(
        select(TournamentAttempt.time_ms, User.handle)
        .join(User, TournamentAttempt.user_id == User.id)
        .where(
            TournamentAttempt.tournament_id == tournament.id,
